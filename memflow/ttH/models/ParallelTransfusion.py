...
#     architecture for conditioning. Uses Parallel Transfusion style conditioning from
...
class BaseCFM(L.LightningModule):
    ...
    def __init__(
        ...
        for _ in range(cfm_args['num_layers']):
            ...
            cfm_layers.append(nn.BatchNorm1d(d_hidden))
            cfm_layers.append(activation_fn())
            d_in = d_hidden
        cfm_layers.append(nn.Linear(d_hidden, self.len_flow_feats))
        cfm_layers.append(nn.BatchNorm1d(self.len_flow_feats))  # Normalise final output
        ...
        for i,(n,reco_features,flow_features) in enumerate(
            zip(self.n_reco_particles_per_type,self.reco_input_features_per_type,self.flow_input_features)
        ):
            assert len(set(flow_features).intersection(set(reco_features))) == len(flow_features),\
                f'Not all flow features {flow_features} found in reco_features for particle set #{i} ({reco_features})'
    ...
    def make_state_embeddings(
            self,
            flow_input_features: List[Union[List[str], Tuple[str, ...]]]
    ) -> nn.ModuleList:
        """
        Creates embedding layers for current state (x_t) feature sets. Inputs with identical structures share the same embedding layers.
        Handles phi differently as phi has already been expaned into (sin, cos) in in x_t, meaning the input dim is
        len(features) + 1 if 'phi' is in the feature set.

        Args:
            - flow_input_features (List[Union[List[str], Tuple[str, ...]]]): List of feature names for each particle type.

        Returns:
            - embeddings (nn.ModuleList): Embedding layers for each particle type.
        """
        ...
        for features in flow_input_features:  # Iterate over each feature set
            ...
            in_dim = len(features)
            if "phi" in features:
                in_dim += 1  # Because phi now has 2 features

            if key not in feature_embeddings:  # If no embedding for this structure
                layers = []
                for i, out_dim in enumerate(self.embed_dims):
                    if i == 0:
    ...
    def conditioning(
            ...
            x_t: torch.Tensor,
            state_mask_exist: List[torch.Tensor],
            t: torch.Tensor
    ) -> torch.Tensor:
        """
        ...
        Args:
            ...
            - state_mask_exist (List[torch.Tensor]): list of masks, one per reco-level particle type, each of shape [B, n_particles].
        ...
        # Process hard data (Transformer encoder input)
        ...
        for i, features in enumerate(self.flow_input_features):
            ...
            slice_state = x_t[:, offset:offset+n, :effective_dim]  # Slice out state for current particle type
            emb_i = self.state_embeddings[i](slice_state) # Apply state embedding for this particle type
            state_emb_list.append(emb_i)
            mask_list.append(state_mask_exist[i])
            offset += n

        # Concatenate state embeddings and masks from all particle types
        ...
        # Time embedding
        if t.dim() == 1: # Ensure t is of shape [B, 1]
            t = t.unsqueeze(1)
        if t.shape[0] != B:
            t = t.transpose(0, 1)
        ...
        # Transformer
        # Encoder: Uses self-attention for hard-level data
        # Decoder: Uses self-attention for current state with time, cross-attention to encoder output
        condition = self.transformer(
            src = hard_emb,                                 # Encoder input
            tgt = state_emb,                                # Decoder input
            tgt_mask = self.tgt_mask.to(hard_emb.device),   # Triangular (causal) mask
            src_key_padding_mask = hard_mask_attn,
            memory_key_padding_mask = hard_mask_attn,
            tgt_key_padding_mask = state_mask_attn,
        )
        condition = self.norm(condition) # Normalise output
    ...
    def compute_velocity(
        ...
        Args:
            - context (torch.Tensor): Transformer context, shape [B, sum_reco, d_model].
    ...
    def forward(
            ...
            x0 = torch.randn_like(x_real)  # Initial state drawn from a Gaussian, shape: [B, sum_reco, len_flow_feats]
            x1 = x_real # Target state
            t = torch.rand(B, device=x_real.device)  # Each batch gets a random time

            # Child classes using OT can perform re-pairing logic if needed
            ...
            # Compute the predicted velocity vector
    ...
    def sample(
        ...
        At each integration step, the Transformer conditioning is re-evaluated based on the current state x_t and the time t.
        ...
        all_trajectories = [] if store_trajectories else None # To store the entire trajectory for each sample, over time steps
        ...
        for s in range(N_sample):
            # Initialise with Gaussian noise
            x0 = torch.randn_like(x_real)  # [B, sum_reco, len_flow_feats]
            x_t = x0.clone()

            if store_trajectories: # (optional) store intermediate states at each integration step
                traj_states = [x_t.detach().cpu().clone()]

            # Perform the integration using the RK4 ODE solver
            dt = 1.0 / steps
            for step_i in range(steps):
                t_value = step_i * dt
                t_ = torch.full((B,), t_value, device=x_t.device)
                # Recompute conditioning using the current state x_t and time t_
                ...
                x_t = self.rk4_step(context, x_t, t_, dt) # Update state using RK4 method

                if store_trajectories: # record partial features for plotting
                    traj_states.append(x_t.detach().cpu().clone())

            if store_trajectories: # Stack the states into single tensor, shape [steps + 1, B, sum_reco, 2]
                traj_states = torch.stack(traj_states, dim=0)  # [steps + 1, B, sum_reco, 2]
                all_trajectories.append(traj_states)

            # Unpack final state into per reco-level particle type samples
            reco_samples = self.unpack_reco_samples(x_t, reco_mask_exist)  # List of [B, particles, features]
            for i in range(N_reco):
                samples[i].append(reco_samples[i])

        # Stack samples for each reco particle type
        samples = [torch.stack(s, dim=0) for s in samples]
