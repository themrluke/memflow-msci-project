...
class MultiAcceptanceDataset(MultiHelperClass,Dataset):
    def __init__(self,datasets):
        ...
        self.match_preprocessing(self.hard_datasets)
        print ('... Re-applying common preprocessing')
        self.do_preprocessing(self.hard_datasets)

        # Compute indices #
        self.compute()

    def __str__(self):
        return f'Acceptance dataset :\n'+'\n'.join([str(dataset) for dataset in self.datasets])
...
class AcceptanceDataset(Dataset):
    ...
    def flatten_dim(self):
        return sum(
            [
                self.hard_dataset.objects[name][0].shape[1]*self.hard_dataset.objects[name][0].shape[2]
                for name in self.hard_dataset.selection
            ]
        )

class MultiMultiplicityDataset(MultiHelperClass,Dataset):
    def __init__(self,datasets):
        ...
        # Check dataset #
        for dataset in self.datasets:
            assert isinstance(dataset,MultiplicityDataset)
        assert len(self.datasets)>0, 'Need to provide at least one dataset'
        ...
        # Match targets size #
        self.N_min = min([dataset.N_min for dataset in self.datasets])
        self.N_max = max([dataset.N_max for dataset in self.datasets])
        for dataset in self.datasets:
            dataset.targets = torch.nn.functional.one_hot(
                dataset.n_objects - self.N_min,
                num_classes = self.N_max - self.N_min + 1,
            ).to(torch.float32)
            dataset.N_min = self.N_min
            dataset.N_max = self.N_max

        # Compute indices #
        self.compute()

    def __str__(self):
        return f'Multiplicity dataset :\n'+'\n'.join([str(dataset) for dataset in self.datasets])
...
class MultiplicityDataset(Dataset):
    ...
    """
    def __init__(self,hard_dataset,reco_dataset,reco_object):
        self.hard_dataset = hard_dataset
        self.reco_dataset = reco_dataset
        ...
        # Make target #
        assert reco_object in reco_dataset.objects.keys(), f'Could not find {reco_object} in reco dataset objects'
        self.n_objects = reco_dataset.objects[reco_object][1][self.reco_idx].sum(dim=1).to(torch.int64)
        self.N_min = self.n_objects.min()
        self.N_max = self.n_objects.max()
        self.targets = torch.nn.functional.one_hot(self.n_objects-self.N_min).to(torch.float32)
        self.weights = (
            (self.targets.sum(dim=0) / self.targets.sum() * self.targets.shape[-1]) * torch.ones((self.targets.shape[0],1)) * self.targets
        ).sum(dim=-1)

    def __len__(self):
        return len(self.hard_idx)

    def __getitem__(self, idx):
        """ Returns the hard-level variables and targets """
        data = self.hard_dataset[self.reco_idx[idx]]
        data['target'] = self.targets[idx]
        if 'weights' in data.keys():
            if isinstance(data['weights'],(list,tuple)):
                for i in range(len(data['weights'])):
                    data['weights'][i] *= self.weights[idx]
            else:
...
class AbsDataset(Dataset,metaclass=ABCMeta):
    ...
    def __init__(self,data,selection,default_features=None,build=False,fit=True,device=None,dtype=None,**kwargs):
        ...
        self.finalize()
        self._standardize()
        if self.processed_path is not None and os.path.exists(self.processed_path) and not self.fit:
    ...
    def __getitem__(self,idx):
        ...
        return {
            ...
            'mask': [self.objects[name][1][idx].to(torch.bool) for name in self.selection],
...
class HardDataset(AbsDataset):
    ...
    def compute_PS_point(self):
        ...
        if isinstance(names,str):
            ...
            raise RuntimeError(f'`final_states_object_name` has to be a string or list of strings')
